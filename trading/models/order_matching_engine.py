...
class OrderMatchingEngine:
    ...
    def match_orders(self) -> NoReturn:
        """
        ...
        while not self.buy_heapq.is_empty() and not self.sell_heapq.is_empty():
            ...
            if best_buy_order.price >= best_sell_order.price:
                ...
                self.complete_transaction(
                    best_sell_order, best_buy_order, best_buy_order.price
                )

            elif isinstance(best_buy_order, LimitOrder) and isinstance(
                best_sell_order, MarketOrder
            ):
                self.match_limit_market_orders(best_buy_order, best_sell_order)

            elif isinstance(best_buy_order, MarketOrder) and isinstance(
                best_sell_order, LimitOrder
            ):
