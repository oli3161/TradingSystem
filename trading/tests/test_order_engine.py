...
def setup_engine():
    ...
    limit_order_buy = LimitOrder("AAPL", price=150.00, quantity=10, client=Client(1), buy_order=True, assets=client1_assets)
    limit_order_sell = LimitOrder("AAPL", price=155.00, quantity=5, client=Client(2), buy_order=False, assets=client2_assets)
    market_order_buy = MarketOrder("AAPL", price=155.00, quantity=5, client=Client(1), buy_order=True, assets=client1_assets)
    market_order_sell = MarketOrder("AAPL", price=150.00, quantity=5, client=Client(2), buy_order=False, assets=client2_assets)

    return engine, stock_listing, limit_order_buy, limit_order_sell, market_order_buy, market_order_sell

def test_add_buy_order(setup_engine):
    engine, _, limit_order_buy, _, _, _ = setup_engine
    engine.add_buy_order(limit_order_buy)
    assert engine.buy_heapq.peek().price == 150.00

def test_add_sell_order(setup_engine):
...
def test_order_matching_limit_to_limit(setup_engine):
...
def test_order_matching_limit_to_market(setup_engine):
    ...
    assert len(engine.transactions) == 1
    transaction = engine.transactions[0]
    assert transaction.price == 150.00  # Transaction price should be the limit order price

def test_order_matching_market_to_limit(setup_engine):
    ...
    assert len(engine.transactions) == 1
    transaction = engine.transactions[0]
    assert transaction.price == 155.00  # Transaction price should be the limit order price

def test_order_matching_market_to_market(setup_engine):
...
def test_partial_fill(setup_engine):
    engine, _, limit_order_buy, limit_order_sell, _, _ = setup_engine
...
def test_no_match(setup_engine):
    engine, _, limit_order_buy, limit_order_sell, _, _ = setup_engine
    engine.add_buy_order(limit_order_buy)
    engine.add_sell_order(limit_order_sell)
...
def test_transaction_update(setup_engine):
    engine, stock_listing, limit_order_buy, limit_order_sell, _, _ = setup_engine
    initial_price = stock_listing.last_price
...
def test_spread_in_market_orders(setup_engine):
    engine, _, _, _, market_order_buy, market_order_sell = setup_engine
    engine.add_buy_order(market_order_buy)
    engine.add_sell_order(market_order_sell)
